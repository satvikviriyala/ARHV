...
| timing | monotonic; median interval 8–200 ms; max gap ≤ 750 ms; duration from baseline + holds to limit + 2 s | fast-forwarded or stitched traces |
...
| gyro | per moving axis, \|corr(d orientation/dt, rotationRate)\| ≥ 0.35 (scale- and sign-free) | dead or random gyroscope |
...
The demo verifier uses an 18° `tiltErrDeg` budget to absorb browser sensor-fusion/calibration skew; the
previous 12° budget was too brittle for that cross-sensor timing. It also accepts the documented low-rate/noisy
mobile limits: gyro correlation down to 0.35 and median delivery intervals up to 200 ms. These allowances do not
change gravity, continuity, fresh-target, single-use token, replay, or Cedar decisions. Never loosen binding,
